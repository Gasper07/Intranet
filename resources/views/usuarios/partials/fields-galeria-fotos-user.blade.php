<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 geleriFotosUser fotoUserFake">
  ...
  @foreach($arrayOfImages as $DataImages => $valuDateImages)
    <p class="gasper">{{ $canditdadImagesForArray = count($DataImages) }}</p>
      <p class="gasper">{{ $GetImage  = \Storage::disk('ubUploadsChange')->get('/documents/'.$DataImages['imagesGalery'].'') }}</p>
      <img class="img-responsive clImgView" src="data:{{ $DataImages['mime'] }};base64,{{ base64_encode($GetImage) }}" alt=""  data-toggle="modal" data-target="#myModalswPost">
        ...
        <div class="modal-dialog dialoDataImgen" role="document">
          <div class="modal-content">
            <div class="modal-body">
              <img class="img-responsive" src="data:{{ $DataImages['mime'] }};base64,{{ base64_encode($GetImage) }}" alt="">
            </div>
